...
export default function EmployeeMensajes() {
  ...
  const contactosAMostrar = administradores.filter(admin => 
    contactosPrevios.includes(admin.id_usuario) || 
    contactoActivo?.id_usuario === admin.id_usuario || 
    (sinLeer[admin.id_usuario] || 0) > 0
  );

  return (
    <div className="flex h-[calc(100vh-4rem)] bg-white dark:bg-slate-800 border-t border-gray-100 dark:border-slate-700 transition-colors relative">
      ...
      <div className={`w-full md:w-80 bg-gray-50/50 dark:bg-slate-900/30 border-r border-gray-100 dark:border-slate-700 p-4 flex flex-col ${contactoActivo ? 'hidden md:flex' : 'flex'}`}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button 
            ...
            aria-label="Nueva Conversación con Admin"
            ...
            title="Nuevo mensaje a Administración"
          >
        ...
        {loading ? (
          ...
          <div className="text-center mt-10 text-slate-400 dark:text-slate-500">
            ...
            <p className="text-xs mt-1">Pulsa en + para contactar a administración.</p>
          </div>
        ) : (
          <div className="space-y-2 overflow-y-auto flex-1 hide-scrollbar">
            {contactosAMostrar.map(admin => {
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black uppercase flex-shrink-0 ${isActive ? 'bg-blue-500 text-white' : 'bg-slate-500 text-white'}`}>
      ...
      {contactoActivo ? (
        ...
        <div className="hidden md:flex flex-1 flex-col items-center justify-center bg-gray-50/30 dark:bg-slate-900/50 text-gray-400 dark:text-slate-500">
          ...
          <p className="text-sm font-bold">Selecciona una conversación para abrir el chat.</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-opacity">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[80vh] border border-gray-100 dark:border-slate-700">
            <div className="p-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900/50">
              <h3 className="font-black text-slate-800 dark:text-white">Contactar Administrador</h3>
              ...
              {administradores
                ...
                >
                  ...
                  <div className="overflow-hidden">
                    <p className="font-bold text-sm text-slate-800 dark:text-white truncate">{admin.nombre}</p>
